...
"use strict";angular.module("asymmetrik.reports").controller("ViewReportController",["$scope","$location","$log","$stateParams","Authentication","authService","Alerts","reportService",function($scope,$location,$log,$stateParams,Authentication,authService,Alerts,reportService){$scope.auth=Authentication,$scope.alertService=Alerts,$scope.alertService.clearAll(),reportService.recentActivity($stateParams.reportId).then(function(result){var processedActivity=reportService.processUserActivitySummary(result);$scope.report=result.report,$scope.users=processedActivity.users,$scope.current=processedActivity.current,$scope.previous=processedActivity.previous},function(){$log.error("Report with id: "+$stateParams.reportId+" does not exist.")});var sort={screenName:{id:"screenName",value:"screenName",dir:!1},created:{id:"created",value:"current.p.createdDate",dir:!0},friends:{id:"friends",value:"current.p.friendsCount",dir:!0},friendsDelta:{id:"friendsDelta",value:"delta.friendsCount",dir:!0},friendsDeltaPercent:{id:"friendsDeltaPercent",value:"delta.friendsPercent",dir:!0},followers:{id:"followers",value:"current.p.followersCount",dir:!0},followersDelta:{id:"followersDelta",value:"delta.followersCount",dir:!0},followersDeltaPercent:{id:"followersDeltaPercent",value:"delta.followersPercent",dir:!0},statuses:{id:"statuses",value:"current.p.statusesCount",dir:!0},statusesDelta:{id:"statusesDelta",value:"delta.statusesCount",dir:!0},statusesDeltaPercent:{id:"statusesDeltaPercent",value:"delta.statusesPercent",dir:!0}};$scope.setSort=function(id){null!=$scope.sort&&$scope.sort.id===id?$scope.sort.dir=!$scope.sort.dir:($scope.sort={},$scope.sort.id=sort[id].id,$scope.sort.value=sort[id].value,$scope.sort.dir=sort[id].dir)},$scope.setSort(sort.screenName.id)}]);
