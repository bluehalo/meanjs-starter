...
"use strict";angular.module("app.core").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise(function($injector){var $state=$injector.get("$state");$state.go("reports.list")})}]);
...
"use strict";angular.module("asymmetrik.reports").config(["$stateProvider",function($stateProvider){$stateProvider.state("report",{"abstract":!0,template:"<ui-view/>"}).state("report.list",{url:"/reports",templateUrl:"app/reports/views/list-reports.client.view.html",controller:"ListReportsController",data:{roles:["user"]}}).state("report.create",{url:"/report/create",templateUrl:"app/reports/views/manage-report.client.view.html",controller:"ManageReportController",data:{roles:["user"],mode:"create"},resolve:{Groups:["groupService",function(groupService){return groupService.list()}]}}).state("report.edit",{url:"/report:reportId/edit",templateUrl:"app/reports/views/manage-report.client.view.html",controller:"ManageReportController",data:{roles:["user"],mode:"edit"},resolve:{Groups:["groupService",function(groupService){return groupService.list()}]}}).state("report.view",{url:"/report:reportId",templateUrl:"app/reports/views/view-report.client.view.html",controller:"ViewReportController",data:{requiresAuthentication:!1}}).state("report.view-user",{url:"/report/user/:screenName",templateUrl:"app/reports/views/view-user-activity.client.view.html",controller:"ViewUserActivityController",data:{requiresAuthentication:!1}})}]);
...
"use strict";angular.module("asymmetrik.reports").controller("ViewReportController",["$scope","$location","$log","$stateParams","Authentication","authService","Alerts","reportService",function($scope,$location,$log,$stateParams,Authentication,authService,Alerts,reportService){$scope.auth=Authentication,$scope.alertService=Alerts,$scope.alertService.clearAll(),reportService.reportActivity($stateParams.reportId).then(function(result){var processedActivity=reportService.processReportActivitySummary(result);$scope.report=result.report,$scope.users=processedActivity.users,$scope.current=processedActivity.current,$scope.previous=processedActivity.previous},function(){$log.error("Report with id: "+$stateParams.reportId+" does not exist.")});var sort={screenName:{id:"screenName",value:"screenName",reverse:!1},created:{id:"created",value:"(null != current.p.createdDate)? current.p.createdDate : 0",reverse:!0},friends:{id:"friends",value:"current.p.friendsCount",reverse:!0},friendsDelta:{id:"friendsDelta",value:"delta.friendsCount",reverse:!0},friendsDeltaPercent:{id:"friendsDeltaPercent",value:"delta.friendsPercent",reverse:!0},followers:{id:"followers",value:"current.p.followersCount",reverse:!0},followersDelta:{id:"followersDelta",value:"delta.followersCount",reverse:!0},followersDeltaPercent:{id:"followersDeltaPercent",value:"delta.followersPercent",reverse:!0},statuses:{id:"statuses",value:"current.p.statusesCount",reverse:!0},statusesDelta:{id:"statusesDelta",value:"delta.statusesCount",reverse:!0},statusesDeltaPercent:{id:"statusesDeltaPercent",value:"delta.statusesPercent",reverse:!0}};$scope.setSort=function(id){null!=$scope.sort&&$scope.sort.id===id?$scope.sort.reverse=!$scope.sort.reverse:($scope.sort={},$scope.sort.id=sort[id].id,$scope.sort.value=sort[id].value,$scope.sort.reverse=sort[id].reverse)},$scope.setSort(sort.screenName.id)}]);
"use strict";angular.module("asymmetrik.reports").controller("ViewUserActivityController",["$scope","$location","$log","$stateParams","Authentication","authService","Alerts","reportService",function($scope,$location,$log,$stateParams,Authentication,authService,Alerts,reportService){function generateData(profiles){var data={};series.forEach(function(s){data[s.key]={key:s.seriesName,color:s.color,values:[]}}),profiles.forEach(function(profile){series.forEach(function(s){data[s.key].values.push({x:new Date(profile.md.ts),y:s.y(profile)})})});var toReturn={};return series.forEach(function(s){toReturn[s.key]=[data[s.key]]}),toReturn}$scope.auth=Authentication,$scope.alertService=Alerts,$scope.alertService.clearAll();var series=[{key:"friends",seriesName:"Friends",y:function(d){return d.p.friendsCount},color:"#2ca02c"},{key:"followers",seriesName:"Followers",y:function(d){return d.p.followersCount},color:"#ff7f0e"},{key:"tweets",seriesName:"Tweets",y:function(d){return d.p.statusesCount},color:"#1f77b4"}];$scope.options={chart:{type:"lineChart",height:200,margin:{top:20,right:80,bottom:40,left:80},useInteractiveGuideline:!0,xScale:d3.time.scale.utc(),xAxis:{tickFormat:d3.time.format("%Y-%m-%d")},yAxis:{tickFormat:d3.format(",d")}}},reportService.userActivity($stateParams.screenName).then(function(result){var processedActivity=reportService.processUserActivitySummary(result);$scope.profiles=processedActivity.profiles,$scope.user=processedActivity.user;var data=generateData($scope.profiles);$scope.friends=data.friends,$scope.followers=data.followers,$scope.tweets=data.tweets},function(){$log.error("No data found for screenName: "+$stateParams.screenName)})}]);
